...
export class CompletionScene extends Phaser.Scene 
{
    ...
    init () 
    {
        ...
        this.text_data = [
            ...
            { text:`You have completed the [color=#275EB7]Family and Relationships ${
                ''}Module[/color].`, x:scale.width/2, y:scale.height/2-180, 
                font:'Font_Main', size:'7.5em'},
            ...
            txt:'Go Home'}).setButton().on('pointerdown', () => {
                this.tweens.add({targets:this.sfx_completion, volume:0, duration:1000})
                    .on('complete', (tweens, targets) => {
                        this.sound.stopAll();
                        this.scene.start('gm_completion');
                    });
            });
    }
